<template>
  <div class="container-video-detail">
    ...
    <router-link :to="`/video/${video.videoSeq}`">
      <button class="go">상세 페이지 가기</button></router-link>
...
export default {
  ...
  data() {
    return {
      videoSeq: this.$route.params.videoSeq,
  ...
  watch: {
    '$route'(to, from) {
      if(to.path!=from.path){
        this.videoSeq = to.params.videoSeq;
  ...
  methods: {
    ...
    goDetail(){
      this.$store.dispatch('video-detail', this.videoSeq);
    }
  },
  filters:{
    stringLength(val){
